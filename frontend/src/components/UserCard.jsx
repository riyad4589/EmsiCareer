			...
			<Link to={`/profile/${user.username}`} className='flex flex-col items-center group'>
				<div className="relative">
					<img
						...
						alt={user.name}
						className='w-24 h-24 rounded-full object-cover mb-4 border-2 border-transparent group-hover:border-success transition-colors duration-200'
					/>
				...
				<h3 className='font-semibold text-lg text-center group-hover:text-success transition-colors duration-200'>{user.name}</h3>
			</Link>
			<p className='text-info text-center mt-1'>{user.headline}</p>
			<button 
				onClick={onConnect}
				disabled={isConnecting}
				className='mt-4 bg-success text-white px-4 py-2 rounded-full hover:bg-success-dark transition-colors w-full flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed'
			>
				{isConnection ? (
					<>
						<Check size={16} />
